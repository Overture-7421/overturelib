...
/**
 * A swerve drivetrain. Concrete robots extend this and supply the modules,
 * kinematics and
 * PathPlanner gains.
 */
public abstract class SwerveChassis extends SwerveBase {
	 ...
	 *
	 * <p>
	 * Defaults to the kinematics answer, which is honest but circular: it is
	 * derived from the same
	 * four modules the detector is trying to catch lying. Override it with the
	 * gyro's angular
	 * velocity, the one measurement a sliding wheel cannot corrupt, and the
	 * detector gets sharper.
	 *
	...
	/**
	 * Returns how hard the robot is being accelerated in the floor plane, for the
	 * collision detector.
	 *
	 * <p>
	 * Zero by default, which leaves collision detection switched off. Override it
	 * with the gyro,
	 ...
	 *
	 * <p>
	 * X and Y rather than all three axes: the Pigeon's reading includes gravity, so
	 * Z sits at
	 ...
	 * @return the estimated pose at that time, empty if it is outside the
	 *         estimator's buffer
	 */
	...
	/**
	 * Feeds a vision pose into the estimator, if vision measurements are being
	 * accepted.
	 *
	...
	/**
	 * Returns how long it has been since the last periodic call, for discretizing
	 * the speeds.
	 *
	 * <p>
	 * Measured rather than taken from {@link RobotConstants#kLoopTime}, because
	 * that constant only
	 * describes reality if robot code drives the CommandScheduler at exactly that
	 * rate. A robot that
	 * leaves the scheduler on the default TimedRobot period gets half the
	 * compensation it needs, and
	 * nothing says so.
	 *
	...
	private void updateOdometry(double periodSeconds) {
		...
		// library.
		skidResult = skidDetector.detect(
	 ...
	 *
	 * <p>
	 * Three separate sets of signals used to carry the same numbers:
	 * LinearX/Y/Angular,
	 * SpeedX/Y/Omega and the DesiredSpeeds array were all the requested speeds,
	 * spelled three ways.
	 * They are one ChassisSpeeds struct now, and the measured speeds are published
	 * beside them, which
	...
	public void periodic() {
		...
		SwerveModuleState[] desiredStates = getKinematics()
				.toSwerveModuleStates(ChassisSpeeds.discretize(targetSpeeds, periodSeconds));
