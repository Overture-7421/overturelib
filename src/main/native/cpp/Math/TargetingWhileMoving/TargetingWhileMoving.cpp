...
TargetingWhileMoving::TargetingWhileMoving(
		DistanceToTravelTimeTable distanceToTravelTime,
		units::second_t accelCompFactor) : distanceToTravelTime(
		distanceToTravelTime), accelCompFactor(accelCompFactor) {

}
;

void TargetingWhileMoving::setTargetLocation(
		frc::Translation2d targetLocation) {
	this->targetLocation = targetLocation;
}

frc::Translation2d TargetingWhileMoving::getMovingTarget(
		const frc::Pose2d &robotPose,
		const frc::ChassisSpeeds &fieldRelativeSpeed,
		const ChassisAccels &fieldRelativeAccel) {
	...
	for (int i = 0; i < 5; i++) {
		units::meter_t virtualGoalX = targetLocation.X()
				- shotTime
						* (fieldRelativeSpeed.vx
								+ fieldRelativeAccel.ax * accelCompFactor);
		units::meter_t virtualGoalY = targetLocation.Y()
				- shotTime
						* (fieldRelativeSpeed.vy
								+ fieldRelativeAccel.ay * accelCompFactor);
		testGoalLocation = { virtualGoalX, virtualGoalY };

		units::meter_t distanceToTestGoal = testGoalLocation.Distance(
				robotLocation);
