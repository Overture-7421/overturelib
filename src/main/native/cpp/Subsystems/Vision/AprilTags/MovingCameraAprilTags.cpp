...
MovingCameraAprilTags::MovingCameraAprilTags(frc::AprilTagFieldLayout *tagLayout,
        SwerveChassis *chassis,
        std::function<frc::Transform3d()> cameraPoseSupplier,
        Config config)
    : AprilTags(tagLayout, chassis, std::move(config)),
      cameraPoseSupplier(std::move(cameraPoseSupplier)) {
}
