...
BlinkEffect::BlinkEffect(LedsManager *leds, LedStripName name,
		frc::Color8Bit color, units::second_t period, bool addRequirement) : color(
		color) {
...
void BlinkEffect::Execute() {
	...
	double blinkMultiplier = std::cos(
			elapsedTime.value() * 2.0 * M_PI * 1.0 / period.value()) * 0.5
			+ 0.5;
