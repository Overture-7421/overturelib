...
SimPhotonVisionManager::SimPhotonVisionManager() {
	simulatedDriveTrainPoseEntry =
			nt::NetworkTableInstance::GetDefault().GetStructTopic < frc::Pose2d
					> ("/AdvantageKit/RealOutputs/FieldSimulation/RobotPosition").Subscribe(
							{ });
...
void SimPhotonVisionManager::AddSimCamera(photon::PhotonCameraSim *camera,
		const frc::Transform3d &robotToCamera) {
	visionSim.AddCamera(camera, robotToCamera);
	std::cout << "Added camera " << camera->GetCamera()->GetCameraName()
			<< " to SimPhotonVisionManager" << std::endl;
}
