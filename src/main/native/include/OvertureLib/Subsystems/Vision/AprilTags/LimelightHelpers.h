	...
	return frc::Pose3d(
			...
			frc::Rotation3d(
					units::angle::radian_t(
							inData[3] * (std::numbers::pi / 180.0)),
					units::angle::radian_t(
							inData[4] * (std::numbers::pi / 180.0)),
					units::angle::radian_t(
...
inline nt::DoubleArrayEntry& getLimelightDoubleArrayEntry(
	...
	if (it == doubleArrayEntries.end()) {
		std::shared_ptr < nt::NetworkTable > table = getLimelightNTTable(
				tableName);
...
inline void setLimelightNTDoubleArray(const std::string &tableName,
		const std::string &entryName, const std::span<const double> &vals) {
...
inline frc::Pose3d getBotPose3d(const std::string &limelightName = "") {
	std::vector<double> poseArray = getLimelightNTDoubleArray(limelightName,
			"botpose");
...
inline frc::Pose2d getBotPose2d_wpiBlue(const std::string &limelightName = "") {
	std::vector<double> result = getLimelightNTDoubleArray(limelightName,
			"botpose_wpiblue");
...
inline void setPythonScriptData(const std::string &limelightName,
	...
	setLimelightNTDoubleArray(limelightName, "llrobot", std::span {
			outgoingPythonData.begin(), outgoingPythonData.size() });
}
...
inline void triggerRewindCapture(const std::string &limelightName,
	...
	std::vector<double> currentArray = getLimelightNTDoubleArray(limelightName,
			"capture_rewind");
...
public:
	...
	RawFiducial(int id, double txnc, double tync, double ta,
			double distToCamera, double distToRobot, double ambiguity) : id(id), txnc(
			txnc), tync(tync), ta(ta), distToCamera(distToCamera), distToRobot(
			distToRobot), ambiguity(ambiguity) {
	}
};
...
public:
	...
	RawTarget(double txnc, double tync, double ta) : txnc(txnc), tync(tync), ta(
			ta) {
	}
};
...
public:
	...
	RawDetection(int classId, double txnc, double tync, double ta,
			...
			double corner3_X, double corner3_Y) : classId(classId), txnc(txnc), tync(
			tync), ta(ta), corner0_X(corner0_X), corner0_Y(corner0_Y), corner1_X(
			corner1_X), corner1_Y(corner1_Y), corner2_X(corner2_X), corner2_Y(
			corner2_Y), corner3_X(corner3_X), corner3_Y(corner3_Y) {
	}
};
...
public:
	...
	PoseEstimate(const frc::Pose2d &pose,
			units::time::second_t timestampSeconds, double latency,
			int tagCount, double tagSpan, double avgTagDist, double avgTagArea,
			const std::vector<RawFiducial> &rawFiducials, bool isMegaTag2) : pose(
			pose), timestampSeconds(timestampSeconds), latency(latency), tagCount(
			tagCount), tagSpan(tagSpan), avgTagDist(avgTagDist), avgTagArea(
			avgTagArea), rawFiducials(rawFiducials), isMegaTag2(isMegaTag2) {
	}
};
...
inline PoseEstimate getBotPoseEstimate(const std::string &limelightName,
		const std::string &entryName, bool isMegaTag2) {
	nt::DoubleArrayEntry &poseEntry = getLimelightDoubleArrayEntry(
	...
	if (poseArray.size() == expectedTotalVals) {
		for (int i = 0; i < tagCount; i++) {
			...
			rawFiducials.emplace_back(id, txnc, tync, ta, distToCamera,
					distToRobot, ambiguity);
		}
	}

	return PoseEstimate(pose, units::time::second_t(adjustedTimestamp), latency,
			tagCount, tagSpan, tagDist, tagArea, rawFiducials, isMegaTag2);
}
/// @endcond
...
inline std::vector<RawFiducial> getRawFiducials(
		const std::string &limelightName = "") {
	std::vector<double> rawFiducialArray = getLimelightNTDoubleArray(
			limelightName, "rawfiducials");
	int valsPerEntry = 7;
...
inline std::vector<RawDetection> getRawDetections(
		const std::string &limelightName = "") {
	std::vector<double> rawDetectionArray = getLimelightNTDoubleArray(
	...
	for (int i = 0; i < numDetections; ++i) {
		int baseIndex = i * valsPerEntry;
		int classId = static_cast<int>(extractEntry(rawDetectionArray,
				baseIndex));
		...
		rawDetections.emplace_back(classId, txnc, tync, ta, corner0_X,
				corner0_Y, corner1_X, corner1_Y, corner2_X, corner2_Y,
				corner3_X, corner3_Y);
	}
...
inline std::vector<RawTarget> getRawTargets(const std::string &limelightName =
		"") {
	std::vector<double> rawTargetArray = getLimelightNTDoubleArray(
			limelightName, "rawtargets");
	int valsPerEntry = 3;
...
inline IMUData getIMUData(const std::string &limelightName) {
	std::vector<double> imuData = getLimelightNTDoubleArray(limelightName,
			"imu");
...
inline void from_json(const wpi::json &j, IMUResults &t) {
	t.data = SafeJSONAccess<std::vector<double>>(j, "data",
			std::vector<double> { });
	t.quaternion = SafeJSONAccess<std::vector<double>>(j, "quat",
...
inline void from_json(const wpi::json &j, RewindStats &t) {
	...
	t.frameCount =
			static_cast<int>(SafeJSONAccess<double>(j, "frameCount", 0.0));
	t.latencyPenalty =
			static_cast<int>(SafeJSONAccess<double>(j, "latpen", 0.0));
...
inline void from_json(const wpi::json &j, RetroreflectiveResultClass &t) {
	...
	t.m_CAMERATransform6DTARGETSPACE = SafeJSONAccess<std::vector<double>>(j,
			internal::_key_transformCAMERAPOSE_TARGETSPACE, defaultValueVector);
	t.m_CAMERATransform6DROBOTSPACE = SafeJSONAccess<std::vector<double>>(j,
			internal::_key_transformCAMERAPOSE_ROBOTSPACE, defaultValueVector);

	t.m_TargetTransform6DCAMERASPACE = SafeJSONAccess<std::vector<double>>(j,
			internal::_key_transformTARGETPOSE_CAMERASPACE, defaultValueVector);
	t.m_TargetTransform6DROBOTSPACE = SafeJSONAccess<std::vector<double>>(j,
			internal::_key_transformTARGETPOSE_ROBOTSPACE, defaultValueVector);
	t.m_ROBOTTransform6DTARGETSPACE = SafeJSONAccess<std::vector<double>>(j,
			internal::_key_transformROBOTPOSE_TARGETSPACE, defaultValueVector);
	t.m_ROBOTTransform6DFIELDSPACE = SafeJSONAccess<std::vector<double>>(j,
			internal::_key_transformROBOTPOSE_FIELDSPACE, defaultValueVector);

	t.m_TargetXPixels = SafeJSONAccess<double>(j, internal::_key_TargetXPixels,
			0.0);
	t.m_TargetYPixels = SafeJSONAccess<double>(j, internal::_key_TargetYPixels,
			0.0);
	t.m_TargetXDegreesCrosshairAdjusted = SafeJSONAccess<double>(j,
			internal::_key_TargetXDegreesCrosshair, 0.0);
	t.m_TargetYDegreesCrosshairAdjusted = SafeJSONAccess<double>(j,
			internal::_key_TargetYDegreesCrosshair, 0.0);
	t.m_TargetAreaNormalized = SafeJSONAccess<double>(j,
			internal::_key_TargetAreaNormalized, 0.0);
...
inline void from_json(const wpi::json &j, FiducialResultClass &t) {
	...
	t.m_fiducialID = static_cast<int>(SafeJSONAccess<double>(j,
			internal::_key_fiducialID, 0.0));
	t.m_CAMERATransform6DTARGETSPACE = SafeJSONAccess<std::vector<double>>(j,
			internal::_key_transformCAMERAPOSE_TARGETSPACE, defaultValueVector);
	t.m_CAMERATransform6DROBOTSPACE = SafeJSONAccess<std::vector<double>>(j,
			internal::_key_transformCAMERAPOSE_ROBOTSPACE, defaultValueVector);
...
inline void from_json(const wpi::json &j, DetectionResultClass &t) {
	...
	t.m_classID = static_cast<int>(SafeJSONAccess<double>(j,
			internal::_key_classID, 0.0));
	...
	t.m_TargetYDegreesCrosshairAdjusted = SafeJSONAccess<double>(j,
			internal::_key_TargetYDegreesCrosshair, 0.0);
	t.m_TargetXDegreesNoCrosshairAdjusted = SafeJSONAccess<double>(j,
			internal::_key_TargetXDegreesNoCrosshair, 0.0);
	t.m_TargetYDegreesNoCrosshairAdjusted = SafeJSONAccess<double>(j,
			internal::_key_TargetYDegreesNoCrosshair, 0.0);
	t.m_TargetAreaNormalized = SafeJSONAccess<double>(j,
			internal::_key_TargetAreaNormalized, 0.0);
	t.m_TargetCorners = SafeJSONAccess < std::vector<std::vector<double>>
			> (j, internal::_key_corners, std::vector<std::vector<double>> { });
...
inline void from_json(const wpi::json &j, VisionResultsClass &t) {
	...
	t.m_latencyPipeline = SafeJSONAccess<double>(j,
			internal::_key_latency_pipeline, 0.0);
	t.m_latencyCapture = SafeJSONAccess<double>(j,
			internal::_key_latency_capture, 0.0);
	t.m_pipelineIndex = SafeJSONAccess<double>(j, internal::_key_pipelineIndex,
			0.0);
	...
	t.timestamp_nt = SafeJSONAccess<long long>(j, internal::_key_timestamp_nt,
			0LL);
	t.timestamp_sys = SafeJSONAccess<long long>(j, internal::_key_timestamp_sys,
			0LL);
	t.timestamp_us = SafeJSONAccess<long long>(j, internal::_key_timestamp_us,
			0LL);

	std::vector<double> defaultVector(6, 0.0);
	t.botPose = SafeJSONAccess<std::vector<double>>(j, internal::_key_botpose,
			defaultVector);
	t.botPose_wpired = SafeJSONAccess<std::vector<double>>(j,
			internal::_key_botpose_wpired, defaultVector);
	t.botPose_wpiblue = SafeJSONAccess<std::vector<double>>(j,
			internal::_key_botpose_wpiblue, defaultVector);
	t.botPose_orb = SafeJSONAccess<std::vector<double>>(j,
			internal::_key_botpose_orb, defaultVector);
	t.botPose_orb_wpiblue = SafeJSONAccess<std::vector<double>>(j,
			internal::_key_botpose_orb_wpiblue, defaultVector);
	t.botPose_orb_wpired = SafeJSONAccess<std::vector<double>>(j,
			internal::_key_botpose_orb_wpired, defaultVector);
	t.botpose_tagcount = SafeJSONAccess<double>(j,
			internal::_key_botpose_tagcount, 0.0);
	t.botpose_span = SafeJSONAccess<double>(j, internal::_key_botpose_span,
			0.0);
	t.botpose_avgdist = SafeJSONAccess<double>(j,
			internal::_key_botpose_avgdist, 0.0);
	t.botpose_avgarea = SafeJSONAccess<double>(j,
			internal::_key_botpose_avgarea, 0.0);
	t.camerapose_robotspace = SafeJSONAccess<std::vector<double>>(j,
			internal::_key_camerapose_robotspace, defaultVector);

	t.pythonOutput = SafeJSONAccess<std::vector<double>>(j,
			internal::_key_pythonOutput, std::vector<double> { });
	t.hardware = SafeJSONAccess<HardwareReport>(j, internal::_key_hw,
			HardwareReport { });
	t.imuResults = SafeJSONAccess<IMUResults>(j, internal::_key_imu,
			IMUResults { });
	t.rewindStats = SafeJSONAccess<RewindStats>(j, internal::_key_rewind,
			RewindStats { });

	t.RetroResults = SafeJSONAccess<std::vector<RetroreflectiveResultClass>>(j,
			"Retro", std::vector<RetroreflectiveResultClass> { });
	t.FiducialResults = SafeJSONAccess<std::vector<FiducialResultClass>>(j,
			"Fiducial", std::vector<FiducialResultClass> { });
	t.DetectionResults = SafeJSONAccess<std::vector<DetectionResultClass>>(j,
			"Detector", std::vector<DetectionResultClass> { });
	t.ClassificationResults = SafeJSONAccess<
			std::vector<ClassificationResultClass>>(j, "Classifier",
			std::vector<ClassificationResultClass> { });
	t.BarcodeResults = SafeJSONAccess<std::vector<BarcodeResultClass>>(j,
			"Barcode", std::vector<BarcodeResultClass> { });
}
 ...
 */
inline LimelightResultsClass getLatestResults(const std::string &limelightName =
		"", bool profile = false) {
	...
	double nanos = std::chrono::duration_cast < std::chrono::nanoseconds
			> (end - start).count();
	double millis = (nanos * 0.000001);
	try {
		LimelightResultsClass out = data.get<
				LimelightHelpers::LimelightResultsClass>();
